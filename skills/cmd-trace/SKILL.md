...
**Example:**
- `/cmd-trace my-project` → `docs/traces/2026-03-02-my-project-trace.md`
...
**操作**: 检查项目 ID 是否有效
**输出**: 验证结果
**错误处理**: 项目不存在时使用 /cmd-status 列出可用项目；项目 ID 格式无效时显示正确格式示例

### Step 2: Collect Traceable Artifacts
**目标**: 收集可追溯制品
**操作**: 扫描项目中的 Intent、Blueprint、Delivery
**输出**: 制品列表
**错误处理**: 无可追溯制品时提示使用 /cmd-init 创建制品并用 # 手动链接工件 (功能计划中) 建立链接；制品链接缺失时记录警告并继续处理其他制品
...
❌ Error: No traceable artifacts in project 'my-project'
   → Create artifacts with '/cmd-init' and link with '# 手动链接工件 (功能计划中)'
...
1. **Verify project**: `/cmd-status`
2. **Check artifacts**: `/cmd-status --project-id=<id>`
3. **Create output directory**: `mkdir -p docs/traces/`
4. **Generate partial trace**: `/cmd-trace <project-id> --partial`
5. **Export to alternative location**: `/cmd-trace <project-id> --output=/tmp/trace.md`
...
```bash
/cmd-trace api-gateway --lang=en-us
```
...
```bash
/cmd-trace microservices-platform
```
...
```bash
/cmd-trace non-existent-project
```
...
Suggestions:
  ...
  - Run: /cmd-status
  - Create new project: /cmd-init --project-id=your-project
```

### Example 6: 边界情况 - 无制品项目

```bash
/cmd-trace empty-project
```
...
Getting Started:
  1. Create Intent: /cmd-init --type=intent --project-id=empty-project
  2. Create Blueprint: /cmd-design --intent-id=INT-001
  3. Create Delivery: /cmd-build --blueprint-id=BLP-001
  4. Generate trace: /cmd-trace empty-project

Alternative:
  - Import from template: /cmd-init --from-template=starter (计划中)
```
