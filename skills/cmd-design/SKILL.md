...
```bash
/cmd-design --name=deploy-skill
/cmd-design --name=api-service --intent-id=INT-001
/cmd-design --name=api-service --intent-id=INT-001 --lang=en-us
```
...
### 调用示例
```
用户: /cmd-design --name=deploy-skill
  ↓
...
**Example:**
- `/cmd-design --name=deploy-skill` → `docs/designs/2026-03-02-deploy-skill-design.md`
...
**Input**: Existing intent from `/cmd-init`
...
```bash
# First design
/cmd-design --name=doc-reader

# Review and iterate on design
/cmd-review --artifact-id=BLP-001

# Create improved version
/cmd-design --name=doc-reader-v2 --intent-id=INT-001
```
