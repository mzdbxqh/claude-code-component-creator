...
### 调用示例
```
用户: /cmd-diff --from=BLP-001 --to=BLP-002
  ↓
...
```bash
/cmd-diff --from=BLP-001 --to=BLP-002
/cmd-diff --from=INT-001 --to=current
/cmd-diff --from=BLP-001 --to=BLP-002 --lang=en-us
```
...
**Example:**
- `/cmd-diff --from=BLP-001 --to=BLP-002` → `docs/ccc/diffs/2026-03-02-BLP-001-diff.md`
...
❌ Error: Source artifact 'BLP-999' not found
   → Use '/cmd-status' to see available artifacts
...
1. **List artifacts**: `/cmd-status --project-id=<id>`
2. **Check artifact details**: `/cmd-status --show-details --artifact-id=<id>`
3. **Compare with different targets**: `/cmd-diff --from=<id1> --to=<id2>`
4. **Export diff for analysis**: `/cmd-diff --from=<id1> --to=<id2> --export=diff.json`
...
```bash
/cmd-diff --from=INT-001 --to=current --export=json
```
...
```bash
/cmd-diff --from=INT-001 --to=BLP-001
```
...
Suggestion:
   - Compare Intent versions: /cmd-diff --from=INT-001 --to=INT-002
   - Compare Blueprint versions: /cmd-diff --from=BLP-001 --to=BLP-002
```
