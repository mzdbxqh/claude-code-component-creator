...
# 运行特定组件测试
/ccc:test-sandbox --target=agents/intent-core

# 运行特定测试用例
/ccc:test-sandbox --test-case=TC-E2E-001
...
```bash
# 生成完整测试报告
/ccc:test-sandbox --report --output=docs/tests/test-report.md

# 生成覆盖率报告
/ccc:test-sandbox --coverage --output=docs/tests/coverage-report.md

# 生成 JUnit 格式报告
/ccc:test-sandbox --junit --output=docs/tests/junit-report.xml
```
...
if echo "$CHANGED_FILES" | grep -q "agents/.*-core/"; then
  ...
  /ccc:test-sandbox --full
elif echo "$CHANGED_FILES" | grep -q "commands/"; then
  echo "命令变更，运行命令测试 + 集成测试"
  /ccc:test-sandbox --target=commands --integration
else
  echo "其他变更，运行相关测试"
  /ccc:test-sandbox --changed
fi
```
...
```bash
# 运行单个测试文件
/ccc:test-sandbox --test-case=TC-INTENT-001

# 运行组件所有测试
/ccc:test-sandbox --target=agents/intent-core

# 运行集成测试
/ccc:test-sandbox --integration
...
# 调试特定阶段
/ccc:test-sandbox --debug --stage=intent
...
# 生成报告
/ccc:test-sandbox --report --output=docs/tests/report.md

# 生成覆盖率
/ccc:test-sandbox --coverage

# 生成 JUnit 报告
/ccc:test-sandbox --junit --output=report.xml
```
