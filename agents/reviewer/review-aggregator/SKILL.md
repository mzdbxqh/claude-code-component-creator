...
from ccc_core.persistence import (
    State, CheckpointManager, DataManager,
    ResumeManager, generate_trace_id
)

# 生成trace_id（如果新会话）
trace_id = generate_trace_id()  # 格式: YYYYMMDD_HHMMSS_Random6
base_dir = ".ccc/persistence"
...
if resume_mgr.can_resume():
    ...
    if response == "从断点恢复":
        # 获取恢复上下文
        ctx = resume_mgr.get_resume_context()
        resume_from_step = ctx.get("current_step", 0) + 1
...
# 保存初始checkpoint
checkpoint_mgr.save("step_0_init", {
    "trace_id": trace_id,
    "phase": "review",
    "step": 0,
    "status": "in_progress",
    "target_path": target_path,
    "timestamp": state.to_dict().get("updated_at")
})

print(f"事务已初始化：{trace_id}")
```
...
**0.4.1 保存组件列表到持久化存储**:
   ...
   ```python
   # 保存扫描结果到持久化存储
   data_mgr.save_result("scan", "components", {
   ...
   # 更新checkpoint
   checkpoint_mgr.save("step_0_scan", {
       "component_count": component_count,
       "parallel_mode": parallel_mode,
       "step": 0
   })
...
checkpoint_mgr.save("step_0_complete", {
    "step": 0,
    "component_count": component_count,
    "parallel_mode": parallel_mode,
    "status": "completed"
...
# 更新状态
state.set_step(0)
state.add_result("component_count", component_count)
state.add_result("parallel_mode", parallel_mode)
```
...
checkpoint_mgr.save("step_1_complete", {
    "step": 1,
...
# 更新状态
state.set_step(1)
state.add_result("reviews_completed", reviews_completed)
state.add_result("reviews_failed", reviews_failed)
```
...
5. **保存验证后的结果到持久化存储**:
   ```python
   # 保存验证后的结果
   data_mgr.save_result("validation", "step_2", {
   ...
   # 更新checkpoint
   checkpoint_mgr.save("step_2_complete", {
       "step": 2,
       "validated_count": validated_count,
       "status": "completed"
   })
   ...
   # 更新状态
   state.set_step(2)
   state.add_result("validated_count", validated_count)
   ```
...
checkpoint_mgr.save("step_7_complete", {
    "step": 7,
...
# 更新状态
state.set_step(7)
state.add_result("architecture_score", architecture_score)
```
...
checkpoint_mgr.save("step_7_5_complete", {
    "step": 7.5,
    "plugin_issues_found": plugin_issues_count,
    "legacy_001_triggered": legacy_001_triggered,
    "status": "completed"
})

# 更新状态
state.set_step(7.5)
state.add_result("plugin_issues_count", plugin_issues_count)
state.add_result("legacy_001_triggered", legacy_001_triggered)
```
...
checkpoint_mgr.save("step_9_complete", {
    "step": 9,
...
# 更新状态
state.set_step(9)
state.add_result("report_generated", True)
state.add_result("report_path", report_path)
state.mark_completed()

# 标记整个workflow为完成状态（保留workflow-engine调用用于状态同步）
Skill(
  skill="ccc:workflow-engine",
