...
**目标**: 加载链路专用反模式规则库
**操作**:
1. 加载 `agents/reviewer/knowledge/antipatterns/linkage/*.yaml` (9条规则)
   - LOOP-001: 循环依赖检测
   - LINK-001, LINK-002, LINK-003: 调用链完整性验证
   - IO-001, IO-002, IO-003: 输入输出匹配检查
   - BRANCH-001, BRANCH-002: 分支完整性检查
2. 解析规则定义（id, severity, description, detection_method, fix_suggestion）
3. 构建验证规则集和检测策略
4. 将规则映射到下一步的验证逻辑
**输出**: 链路验证规则集合（结构化数据）
**错误处理**: 规则文件缺失时记录警告，使用默认验证逻辑；规则解析失败时跳过该规则

### Step 4: 验证链路规则
**目标**: 使用加载的规则检查调用链路是否符合规范
**操作**:

**基于加载规则的验证**:
1. 执行 LOOP-001 循环依赖检测
2. 执行 LINK-* 调用链完整性检查
3. 执行 IO-* 输入输出匹配验证
4. 执行 BRANCH-* 分支完整性检查
