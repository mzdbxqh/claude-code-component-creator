...
**目标**: 加载架构专用反模式规则库
**操作**:
1. 加载 `agents/reviewer/knowledge/antipatterns/architecture/*.yaml` (21条规则)
   - ARCH-001~ARCH-021: 工作流完整性、组件设计、协作模式、命名规范等
2. 解析规则定义（id, severity, description, detection_method, fix_suggestion）
3. 构建规则索引和检测策略
4. 按维度分类规则（工作流/组件/职责/协作/命令）
**输出**: 架构反模式规则集合（结构化数据）
**错误处理**: 规则文件缺失时记录警告，使用已加载规则继续；规则解析失败时跳过该规则

### Step 2: 工作流架构分析
**目标**: 评估工作流设计质量
**操作**:

**Step 2.1: 从cmd-* skills的description提取工作流结构（新增-三层防护体系-评审环节）**
